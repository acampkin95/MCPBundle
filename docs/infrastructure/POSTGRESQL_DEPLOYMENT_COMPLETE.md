...
```
Username: replicator
Password: Contabo secret `mcp-db-repl-password` (export as `DB_REPL_PASSWORD`)
...
# Connection string
postgresql://mcp_admin:${DB_ADMIN_PASSWORD}@46.250.241.70:5432/mcp_ecosystem
```
...
> Export `MCP_ROOT_PASSWORD` via `npm run secrets:pull -- --out .env.secrets && source .env.secrets` before running the following commands:

```bash
# Check replication status on primary
SSHPASS="$MCP_ROOT_PASSWORD" sshpass -e ssh root@46.250.243.123 \
  "sudo -u postgres psql -c 'SELECT * FROM pg_stat_replication;'"

# Check recovery status on standby
SSHPASS="$MCP_ROOT_PASSWORD" sshpass -e ssh root@46.250.241.70 \
...
psql postgresql://mcp_admin:${DB_ADMIN_PASSWORD}@46.250.243.123:5432/mcp_ecosystem -c "SELECT 'Connection successful' as status;"
```
