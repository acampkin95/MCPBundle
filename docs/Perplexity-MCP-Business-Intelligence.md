...
class PerplexityClient {
  ...
  async search(query, options = {}) {
    ...
    try {
      ...
      console.log(`[Perplexity] Query #${this.queryCount}: $${tokenCost.toFixed(4)} | Total: $${this.totalCost.toFixed(2)}`);
...
class NextcloudClient {
  ...
  async listDirectory(dirPath) {
    try {
      const response = await axios.propfind(
        `${this.baseURL}${dirPath}`,
        {},
        { auth: this.auth }
      );
...
server.tool('business_opportunity', {
  ...
  execute: async (input) => {
    ...
    await nextcloud.uploadJSON(
      `/opportunities/opportunities_${Date.now()}.json`,
      research
    );
...
server.tool('business_operations_advisor', {
  ...
  execute: async (input) => {
    ...
    await nextcloud.uploadJSON(
      `/operations/blueprint_${Date.now()}.json`,
      blueprint
    );
