...
test_infrastructure_connectivity() {
    ...
    if ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=accept-new "dev-admin@${VMI01_HOST}" "echo 'SSH OK'" &>/dev/null; then
...
test_database_schema() {
    ...
    for table in "${REQUIRED_TABLES[@]}"; do
        TABLE_EXISTS=$(ssh "dev-admin@${VMI01_HOST}" \
            "PGPASSWORD='${MCP_DB_PASSWORD}' psql -h localhost -U mcp_admin -d mcp_ecosystem -tAc \"SELECT COUNT(*) FROM pg_tables WHERE tablename='$table'\"" 2>/dev/null || echo "0")
    ...
    FUNC_EXISTS=$(ssh "dev-admin@${VMI01_HOST}" \
        "PGPASSWORD='${MCP_DB_PASSWORD}' psql -h localhost -U mcp_admin -d mcp_ecosystem -tAc \"SELECT COUNT(*) FROM pg_proc WHERE proname='search_thoughts'\"" 2>/dev/null || echo "0")
...
EOF
    ...
    if echo "$RESPONSE" | jq -e '.thought_id' &>/dev/null; then
        ...
        THOUGHT_EXISTS=$(ssh "dev-admin@${VMI01_HOST}" \
            "PGPASSWORD='${MCP_DB_PASSWORD}' psql -h localhost -U mcp_admin -d mcp_ecosystem -tAc \"SELECT COUNT(*) FROM thoughts WHERE thought_id='$THOUGHT_ID'\"" 2>/dev/null || echo "0")
...
test_failure_recovery() {
    ...
    QUEUE_SIZE=$(ssh "dev-admin@${VMI01_HOST}" \
        "PGPASSWORD='${MCP_DB_PASSWORD}' psql -h localhost -U mcp_admin -d mcp_ecosystem -tAc \"SELECT COUNT(*) FROM thought_sync_queue WHERE sync_status='pending'\"" 2>/dev/null || echo "unknown")
...
test_performance_metrics() {
    ...
    ssh "dev-admin@${VMI01_HOST}" \
        "PGPASSWORD='${MCP_DB_PASSWORD}' psql -h localhost -U mcp_admin -d mcp_ecosystem -tAc \"SELECT COUNT(*) FROM thoughts\"" &>/dev/null
    END_TIME=$(date +%s%N)
