...
import type {
  ...
  DatabaseDiagnosticSuite,
} from '../services/databaseDiagnostics.js';
...
import type {
  ...
  ThoughtRecord,
} from '../services/structuredThinking.js';
...
const formatThinkingResult = (title: string, result: ThoughtTrackingResult): string => {
  ...
  if (result.branchInsights.length) {
    ...
    for (const branch of result.branchInsights) {
      const average =
        typeof branch.averageQuality === 'number' ? branch.averageQuality.toFixed(2) : 'n/a';
      lines.push(
...
const thoughtMetadataSchema = z
  .object({
    ...
    external_refs: z.array(z.string().min(1)).optional(),
...
export function registerTools(server: McpServer, deps: ToolDependencies): void {
  ...
  server.registerTool(
    ...
    async ({ suites }) => {
      try {
        const results = await deps.databaseDiagnostics.runLocal(
          suites as DatabaseDiagnosticSuite[]
        );
    ...
    'postgres-manage',
    {
      ...
      inputSchema: {
        ...
        database: z
          .string()
          .optional()
          .refine((val) => !val || validateDatabaseName(val), {
            message:
              'Invalid database name. Must be alphanumeric with underscores, starting with letter or underscore, max 63 characters.',
          }),
        minDurationMs: z
          .number()
          .int()
          ...
          .max(3600000)
          .optional()
          .describe('Minimum query duration in ms for slow-queries (max 1 hour)'),
        analyze: z.boolean().optional().describe('Run ANALYZE with VACUUM'),
        destination: z
          .string()
          .optional()
          .refine((val) => !val || validateFilePath(val, ALLOWED_BACKUP_DIRS), {
      ...
      try {
        ...
        switch (operation) {
          ...
          case 'bloat': {
            ...
            sections['Tables with Bloat'] =
              bloat.length > 0
                ? bloat
                    .map(
                      (t) =>
                        `${t.schemaName}.${t.tableName}: ${t.bloatPct}% (${t.deadTuples} dead tuples)`
                    )
                    .join('\n')
          ...
          case 'index-usage': {
            ...
            sections['Unused Indexes'] =
              unused.length > 0
                ? unused
                    .map(
                      (i) =>
                        `${i.schemaName}.${i.tableName}.${i.indexName} (${i.indexSizeMB}MB, 0 scans)`
                    )
                    .join('\n')
        ...
        safeCaptureReport(deps.reportingHub, {
          ...
          external_refs: database ? [database] : undefined,
  ...
  server.registerTool(
    ...
    {
      description:
        'Direct Redis management operations (info, memory, keyspace, slow log, clients, save).',
      inputSchema: {
        ...
        count: z
          ...
          .max(1000)
          .optional()
          .describe('Number of entries for slow-log'),
      },
    },
        ...
        switch (operation) {
          ...
          case 'keyspace': {
            ...
            sections['Databases'] =
              keyspace.length > 0
                ? keyspace
                    .map((db) => `DB${db.database}: ${db.keys} keys, ${db.expires} expires`)
                    .join('\n')
          ...
          case 'slow-log': {
            ...
            sections['Slow Commands'] =
              slowLog.length > 0
                ? slowLog
                    .map(
                      (entry) =>
                        `${entry.duration}µs: ${entry.command} (client: ${entry.clientAddress})`
                    )
                    .join('\n')
                : 'No slow commands';
            result = slowLog;
          ...
          case 'clients': {
            ...
            sections['Client Details'] =
              clients.length > 0
                ? clients
                    .map((c) => `${c.address} (db${c.db}): ${c.cmd}, idle ${c.idle}s`)
                    .join('\n')
  ...
  server.registerTool(
    ...
    {
      description:
        'Keycloak Admin API operations (realm stats, sessions, clients, events, users, secret rotation).',
      inputSchema: {
        operation: z.enum([
          'realm-stats',
          'sessions',
      ...
      try {
        ...
        switch (operation) {
          ...
          case 'sessions': {
            ...
            sections['Session Details'] =
              sessions.length > 0
                ? sessions
                    .slice(0, 10)
                    .map(
                      (s) =>
                        `${s.username} from ${s.ipAddress} (${Object.keys(s.clients).length} clients)`
                    )
                    .join('\n')
                : 'No active sessions';
            result = sessions;
          ...
          case 'events': {
            ...
            sections['Recent Events'] = events.recentEvents
              .slice(0, 5)
              .map((e) => `${new Date(e.time).toISOString()}: ${e.type}`)
              .join('\n');
            result = events;
            break;
          }
          ...
          case 'users': {
            ...
            sections['User List'] = users
              .slice(0, 20)
              .map(
                (u) =>
                  `${u.username} (${u.email ?? 'no email'}) - ${u.enabled ? 'enabled' : 'disabled'}`
              )
              .join('\n');
            result = users;
            break;
        ...
        safeCaptureReport(deps.reportingHub, {
          ...
          external_refs: realm ? [realm] : undefined,
  ...
  server.registerTool(
    ...
    {
      description:
        'NGINX monitoring operations (access log stats, error log, config test, reload).',
      inputSchema: {
        operation: z.enum(['access-stats', 'error-log', 'test-config', 'reload']),
        logPath: z
          .string()
          .optional()
          .refine((val) => !val || validateFilePath(val, ALLOWED_LOG_DIRS), {
    ...
    async ({ operation, logPath, lastMinutes, lines }) => {
      try {
        ...
        safeCaptureReport(deps.reportingHub, {
          ...
          external_refs: logPath ? [logPath] : undefined,
  ...
  server.registerTool(
    ...
    {
      description:
        'Linux system metrics (overview, processes, disk I/O, network, journal errors, service status).',
      inputSchema: {
        operation: z.enum([
          'overview',
          'processes',
        ...
        lastMinutes: z
          ...
          .max(1440)
          .optional()
          .describe('Time window for journal errors'),
        serviceName: z
          .string()
          .optional()
          .refine((val) => !val || validateServiceName(val), {
            message:
              'Invalid service name. Must be from allowed list: postgresql, redis, nginx, docker, pm2, ssh, etc.',
          })
      ...
      try {
        ...
        switch (operation) {
          case 'overview': {
            ...
            sections['Load Average'] =
              `${overview.loadAverage.oneMin} / ${overview.loadAverage.fiveMin} / ${overview.loadAverage.fifteenMin}`;
            sections['Memory'] =
              `${Math.round(overview.memory.used / 1024 / 1024 / 1024)}GB / ${Math.round(overview.memory.total / 1024 / 1024 / 1024)}GB (${overview.memory.percentUsed}%)`;
            sections['Disk Usage'] = overview.disk
              .map((d) => `${d.mountPoint}: ${d.percentUsed}% (${d.used} / ${d.size})`)
              .join('\n');
          ...
          case 'processes': {
            ...
            sections['Top Processes'] = processes
              .map((p) => `${p.command}: ${p.cpu}% CPU, ${p.mem}% MEM (PID ${p.pid})`)
              .join('\n');
            result = processes;
            break;
          }
          ...
          case 'disk-io': {
            ...
            sections['Disk I/O'] =
              diskIO.length > 0
                ? diskIO
                    .map(
                      (d) => `${d.device}: ${d.tps} TPS, ${d.readPerSec} r/s, ${d.writePerSec} w/s`
                    )
                    .join('\n')
          ...
          case 'network': {
            ...
            sections['Network Interfaces'] = network
              .map(
                (n) =>
                  `${n.interface}: RX ${Math.round(n.rxBytes / 1024 / 1024)}MB, TX ${Math.round(n.txBytes / 1024 / 1024)}MB`
              )
              .join('\n');
            result = network;
            break;
      ...
      try {
        switch (parsed.operation) {
          case 'capture': {
            ...
            const result = deps.structuredThinking.trackThoughts(
              entries,
              parsed.autoNumbering ?? true
            );
            return {
              content: [
                {
