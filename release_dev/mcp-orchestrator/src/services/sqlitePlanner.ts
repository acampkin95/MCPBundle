...
export class SQLitePlannerService {
  ...
  public constructor(dbPath: string = join(process.cwd(), "mcp_cache.db")) {
  ...
  /**
   * Get all thoughts from the database (for sync)
   */
  public getAllThoughts(): ThoughtRecord[] {
  ...
  /**
   * Upsert a thought (insert or update if exists)
   */
  public upsertThought(thought: ThoughtRecord): void {
    const stmt = this.db.prepare(`
      INSERT INTO thoughts (id, stage, thought, timestamp, ordering, metadata)
      VALUES (@id, @stage, @thought, @timestamp, @order, @metadata)
    ...
    stmt.run({
      ...
      order: thought.order,
      metadata: thought.metadata ? JSON.stringify(thought.metadata) : null,
    });
  }
}
