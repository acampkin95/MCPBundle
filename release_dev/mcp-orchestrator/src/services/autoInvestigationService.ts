...
export class AutoInvestigationService {
  ...
  public async evaluate(
    ...
    if (this.pm2ProcessName && this.looksLikePm2Failure(payload)) {
      return this.restartPm2(payload);
    }
  ...
  private isSecurityEvent(payload: StructuredLogRecord): boolean {
    const haystack =
      `${payload.message} ${(payload.metadata?.details as string | undefined) ?? ''}`.toLowerCase();
  ...
  private async restartPm2(payload: StructuredLogRecord): Promise<AutoInvestigationOutcome> {
  ...
  private handleSecurityEvent(payload: StructuredLogRecord): AutoInvestigationOutcome {
    const attemptedAt = new Date().toISOString();
    const actor = this.extractActor(payload);
    ...
    return {
      ...
      investigation: {
        ...
        notes: `Security heuristics flagged ${actor ?? 'unknown actor'}`,
      },
    };
  }

  private extractActor(payload: StructuredLogRecord): string | undefined {
    if (typeof payload.metadata?.actor === 'string') {
      return payload.metadata.actor;
    }
    if (typeof payload.metadata?.username === 'string') {
      return payload.metadata.username;
    }
    return undefined;
  }

  private extractSourceIp(payload: StructuredLogRecord): string | undefined {
    if (typeof payload.metadata?.ip === 'string') {
      return payload.metadata.ip;
    }
    if (typeof payload.metadata?.sourceIp === 'string') {
      return payload.metadata.sourceIp;
    }
    return undefined;
  }
