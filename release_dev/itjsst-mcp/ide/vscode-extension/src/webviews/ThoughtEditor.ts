...
import type { StructuredThought, ThoughtMetadata } from '../types';
import type { DatabaseService } from '../services/DatabaseService';

interface ThoughtEditorMessage {
  readonly type: 'save' | 'cancel' | 'preview-ready' | 'error';
  readonly data?: {
...
export class ThoughtEditor {
  private static currentPanel: ThoughtEditor | undefined;
  private readonly panel: vscode.WebviewPanel;
  private readonly thought: StructuredThought;
  ...
  public static async createOrShow(
    ...
    ThoughtEditor.currentPanel = new ThoughtEditor(
      panel,
      thought,
      databaseService,
      outputChannel
   ...
   */
  private updateThought(thought: StructuredThought): void {
   ...
   * Handle messages from webview
   */
  private async handleMessage(message: ThoughtEditorMessage): Promise<void> {
    switch (message.type) {
      case 'save':
      ...
      case 'error':
        void vscode.window.showErrorMessage(
          `Editor error: ${JSON.stringify(message.data)}`
        );
        break;
    }
  }
  ...
  private async handleSave(
    data?: ThoughtEditorMessage['data']
  ): Promise<void> {
    if (!data?.content) {
    ...
    try {
      ...
      this.outputChannel.appendLine(
        `[ThoughtEditor] Saved thought: ${this.thought.thoughtId}`
      );
      ...
      void vscode.window.showErrorMessage(
        `Failed to save thought: ${String(error)}`
      );
      this.outputChannel.appendLine(
        `[ThoughtEditor] Save error: ${String(error)}`
      );
    }
  }
  ...
  <div class="container">
    ...
    <div class="split-container">
      <div class="editor-pane">
        ...
        <textarea id="markdown-input" placeholder="Enter your thought content...">${
        ...
        <div class="metadata-editor">
          <div class="metadata-field">
            ...
            <div>
              <input type="range" min="0" max="100" value="${
                this.thought.qualityScore ?? 50
              }" id="quality-score" />
              <span class="quality-display" id="quality-display">${
                this.thought.qualityScore ?? 50
              }</span>
            </div>
          </div>
          <div class="metadata-field">
            ...
            <select id="importance">
              <option value="low" ${
                this.thought.metadata?.importance === 'low' ? 'selected' : ''
              }>Low</option>
              <option value="medium" ${
                this.thought.metadata?.importance === 'medium' ? 'selected' : ''
              }>Medium</option>
              <option value="high" ${
                this.thought.metadata?.importance === 'high' ? 'selected' : ''
              }>High</option>
              <option value="critical" ${
                this.thought.metadata?.importance === 'critical'
                  ? 'selected'
                  : ''
              }>Critical</option>
            </select>
          ...
          <div class="metadata-field">
            ...
            <input type="text" class="tags-input" id="tags" value="${
              this.thought.metadata?.tags?.join(', ') ?? ''
