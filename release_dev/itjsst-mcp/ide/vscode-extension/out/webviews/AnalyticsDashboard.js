...
class AnalyticsDashboard {
    ...
    disposables = [];
    refreshInterval;
    constructor(panel, databaseService, outputChannel) {
        this.panel = panel;
     ...
     */
    static async createOrShow(extensionUri, databaseService, outputChannel) {
        ...
        const panel = vscode.window.createWebviewPanel('analyticsConsole', 'Thinking Analytics', column, {
            enableScripts: true,
            retainContextWhenHidden: true,
            localResourceRoots: [extensionUri],
        });
        AnalyticsDashboard.currentPanel = new AnalyticsDashboard(panel, databaseService, outputChannel);
    }
    /**
     * Handle messages from webview
     */
    ...
    async update() {
        try {
            ...
            this.outputChannel.appendLine(`[AnalyticsDashboard] Error updating metrics: ${String(error)}`);
            void vscode.window.showErrorMessage(`Failed to update analytics: ${String(error)}`);
        }
    }
    /**
    ...
    async calculateMetrics() {
        ...
        const allThoughts = await Promise.all(sessions.map((s) => this.databaseService.getThoughts(s.sessionId, 1000)));
        const thoughts = allThoughts.flat();
        // Calculate basic metrics
        const totalSessions = sessions.length;
        const totalThoughts = thoughts.length;
        const averageThoughtsPerSession = totalSessions > 0 ? totalThoughts / totalSessions : 0;
        ...
        const completedSessions = sessions.filter((session) => {
            const sessionThoughts = thoughts.filter((t) => t.sessionId === session.sessionId);
            return sessionThoughts.some((t) => t.stage === 'conclusion' || t.stage === 'validation');
        });
    ...
    async exportMetrics() {
        try {
            ...
            void vscode.window.showErrorMessage(`Failed to export analytics: ${String(error)}`);
        }
    }
    /**
     * Generate HTML content for dashboard
     */
    getHtmlContent(metrics) {
