...
class SearchService {
    ...
    async search(filters) {
        ...
        this.logger.appendLine(`[SearchService] Searching with filters: ${JSON.stringify(filters)}`);
        try {
            const mode = this.database.getMode();
            const thoughts = mode === 'postgresql'
                ? await this.searchPostgreSQL(filters)
                : await this.searchSQLite(filters);
            const executionTime = Date.now() - startTime;
            this.logger.appendLine(`[SearchService] Found ${thoughts.length} results in ${executionTime}ms`);
     ...
     */
    async searchPostgreSQL(filters) {
        const conditions = [];
        const params = [];
        let paramIndex = 1;
        ...
        if (filters.hasParent !== undefined) {
            conditions.push(filters.hasParent ? 'parent_thought_id IS NOT NULL' : 'parent_thought_id IS NULL');
        }
    ...
    async searchSQLite(filters) {
        ...
        LIMIT ?
      `;
            const rows = await this.database.query(ftsQuery, [filters.query, filters.limit ?? 100]);
     ...
     */
    async checkFTS5Support() {
        try {
            const result = await this.database.query(`SELECT name FROM sqlite_master WHERE type='table' AND name='thoughts_fts'`, []);
     ...
     */
    async initializeFTS5() {
        if (this.database.getMode() !== 'sqlite') {
            return;
        }
        try {
            await this.database.query(`CREATE VIRTUAL TABLE IF NOT EXISTS thoughts_fts
         USING fts5(content, content='structured_thoughts', content_rowid='rowid')`, []);
            // Create triggers to keep FTS in sync
            await this.database.query(`CREATE TRIGGER IF NOT EXISTS thoughts_ai AFTER INSERT ON structured_thoughts BEGIN
           INSERT INTO thoughts_fts(rowid, content) VALUES (new.rowid, new.content);
         END`, []);
            await this.database.query(`CREATE TRIGGER IF NOT EXISTS thoughts_ad AFTER DELETE ON structured_thoughts BEGIN
           INSERT INTO thoughts_fts(thoughts_fts, rowid, content) VALUES('delete', old.rowid, old.content);
         END`, []);
            await this.database.query(`CREATE TRIGGER IF NOT EXISTS thoughts_au AFTER UPDATE ON structured_thoughts BEGIN
           INSERT INTO thoughts_fts(thoughts_fts, rowid, content) VALUES('delete', old.rowid, old.content);
           INSERT INTO thoughts_fts(rowid, content) VALUES (new.rowid, new.content);
         END`, []);
