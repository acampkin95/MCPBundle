...
Configure LOG_AGGREGATOR_URL, LOG_AGGREGATOR_TOKEN, and an optional MCP_NODE_ID to stream the macOS automation agent logs to the control plane. Set PM2_PROCESS_NAME when the CLI runs under PM2 so self-healing actions can restart the right process.
