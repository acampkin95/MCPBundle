...
export interface QualityFlag {
  ...
  readonly category: "completeness" | "coherence" | "depth" | "repetition" | "vagueness" | "missing-evidence";
...
export class MetacognitiveFeedbackService {
  ...
  private assessCompleteness(thought: ThoughtRecord, wordCount: number): number {
    ...
    if (thought.metadata?.references && thought.metadata.references.length > 0) score += 0.2;
  ...
  private assessCoherence(thought: ThoughtRecord, priorThoughts: readonly ThoughtRecord[]): number {
    ...
    const references = thought.metadata?.references ?? [];
  ...
  private assessNovelty(thought: ThoughtRecord, priorThoughts: readonly ThoughtRecord[]): number {
    ...
    for (const prior of priorThoughts.slice(-5)) { // Check last 5 thoughts
      const priorWords = new Set(prior.thought.toLowerCase().split(/\s+/).filter(w => w.length > 3));
  ...
  private assessEvidence(thought: ThoughtRecord, content: string): number {
    ...
    const references = thought.metadata?.references?.length ?? 0;
  ...
  private computeOverallScore(stage: string, dimensions: QualityDimensions): number {
    ...
    const weights = Object.keys(dimensions).reduce((acc, key) => {
  ...
  private detectQualityIssues(
    ...
    if ((thought.stage === "research" || thought.stage === "analysis") && dimensions.evidenceBased < 0.4) {
      flags.push({
        type: "warning",
  ...
  private generateSuggestions(
    ...
    if (dimensions.specificity < 0.5) {
      suggestions.push("Add specific examples, numbers, or file references to make this more concrete");
    }

    if (dimensions.coherence < 0.5) {
      suggestions.push("Reference prior thoughts or explain how this connects to previous reasoning");
    }

    if (dimensions.evidenceBased < 0.5 && (thought.stage === "research" || thought.stage === "analysis")) {
  ...
  private generateRecommendations(
    ...
    if (overallQuality < 0.5) {
      recommendations.push("Overall thought quality is low. Consider slowing down and providing more detail.");
    }
    ...
    for (const trend of decliningTrends) {
      if (trend.dimension === "coherence") {
        recommendations.push("Coherence is declining. Ensure each thought connects to prior reasoning.");
      }
