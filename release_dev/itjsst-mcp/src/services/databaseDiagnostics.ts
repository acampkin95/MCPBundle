...
export class DatabaseDiagnosticsService {
  ...
  public async runRemote(
    options: RemoteDatabaseOptions & { suites: DatabaseDiagnosticSuite[] }
  ...
  private async runLocalCommands(commands: DiagnosticCommand[]): Promise<DiagnosticResult[]> {
    ...
    for (const command of commands) {
      const output = await this.runner.run(command.command, {
        requiresSudo: command.requiresSudo ?? false,
      });
