...
export class ZteRouterService {
  ...
  private async getStatus(options: ZteConnectionOptions): Promise<ZteResult> {
    ...
    const cmd =
      'wan_ipaddr,network_type,network_provider,cell_id,rssi,rsrp,rsrq,sinr,wan_active_band';
  ...
  private async getNetworkInfo(options: ZteConnectionOptions): Promise<ZteResult> {
    const cmd =
      'network_type,network_provider,wan_active_band,cell_id,pci,rmcc,rmnc,roam_setting_option';
    const result = await this.apiGet(options, cmd);
  ...
  private async getWanInfo(options: ZteConnectionOptions): Promise<ZteResult> {
    const cmd =
      'wan_ipaddr,ipv6_wan_ipaddr,wan_gateway,ipv6_wan_gateway,prefer_dns_manual,prefer_dns_auto';
    const result = await this.apiGet(options, cmd);
  ...
  private async getDeviceInfo(options: ZteConnectionOptions): Promise<ZteResult> {
    const cmd =
      'imei,wan_active_band,hardware_version,web_version,wa_inner_version,cr_version,model_name';
    const result = await this.apiGet(options, cmd);
  ...
  private async getDataUsage(options: ZteConnectionOptions): Promise<ZteResult> {
    const cmd =
      'monthly_rx_bytes,monthly_tx_bytes,monthly_time,data_volume_limit_switch,data_volume_limit_size';
    const result = await this.apiGet(options, cmd);
  ...
  public listOperations(): Array<{
    operation: ZteOperation;
    description: string;
    writesData: boolean;
  }> {
    return [
      {
        operation: 'status',
        description: 'Get comprehensive device status (signal, network, IP)',
        writesData: false,
      },
      {
        operation: 'signal-strength',
        description: 'Get detailed signal metrics (RSSI, RSRP, RSRQ, SINR)',
        writesData: false,
      },
      {
        operation: 'network-info',
        description: 'Get network registration info (operator, band, cell ID)',
        writesData: false,
      },
      {
        operation: 'wan-info',
        description: 'Get WAN IP, gateway, DNS settings',
        writesData: false,
      },
      {
        operation: 'device-info',
        description: 'Get IMEI, model, firmware version',
        writesData: false,
      },
      {
        operation: 'connected-devices',
        description: 'List connected DHCP clients',
        writesData: false,
      },
      {
        operation: 'wifi-status',
        description: 'Get WiFi SSID, password, connected clients',
        writesData: false,
      },
      {
        operation: 'wifi-config',
        description: 'Update WiFi SSID and password (requires ssid, wifiPassword)',
        writesData: true,
      },
      ...
      {
        operation: 'sms-send',
        description: 'Send SMS (requires phoneNumber, smsMessage)',
        writesData: true,
      },
      {
        operation: 'data-usage',
        description: 'Get monthly data usage statistics',
        writesData: false,
      },
      { operation: 'apn-settings', description: 'Get APN configuration', writesData: false },
      {
        operation: 'band-selection',
        description: 'Set 5G/LTE band preference (requires bandMode)',
        writesData: true,
      },
