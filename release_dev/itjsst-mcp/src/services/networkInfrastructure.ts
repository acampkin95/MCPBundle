...
export class NetworkInfrastructureService {
    ...
    const steps: PathTraceStep[] = [
      {
        ...
        commands: [`ip addr show`, `ip route get ${destination}`, `ping -c 3 ${destination}`],
        expectedResult: 'Source host has valid IP configuration and can resolve destination route.',
      },
      {
        layer: 'switch',
        description:
          'Check VLAN trunk port status and spanning-tree state on access/aggregation switches.',
        commands: ['show interfaces status', 'show spanning-tree active', 'show vlan brief'],
      ...
      },
      {
        ...
        commands: ['show ip route', 'show ip bgp summary', 'show ip ospf neighbor'],
      },
    ];
  ...
  public validateFirewallRules(
    ...
    const suggestions: FirewallRuleSuggestion[] = policy.map((rule, index) => ({
      ...
      recommendation:
        'Refine to specific applications, subnets, and user groups. Add logging for denied traffic.',
    }));
