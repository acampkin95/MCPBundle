  ...
  public buildDecomposition(input: TaskDecompositionInput): TaskDecompositionResult {
    const stages =
      input.stages && input.stages.length > 0
        ? input.stages
        : ['Understand requirements', 'Design solution', 'Implement', 'Validate'];
      ...
      activities.push(
        {
          ...
          description:
            'Interview stakeholders or review project brief to nail down success metrics.',
          category: 'report',
      ...
      activities.push(
        {
          ...
          description:
            'Assign separate owners to independent subcomponents to maximise throughput.',
          category: 'build',
          ciStage: 'build',
        },
        {
          ...
          description:
            'Ensure logging/metrics are in place to support debugging during this phase.',
          category: 'ops',
  ...
  private enrichTasksFromThoughts(
    ...
    for (const [branchId, branchThoughts] of branchGroups.entries()) {
      ...
      debugTracks.push({
        ...
        hypothesis: branchThoughts[0].metadata?.external_refs?.join(', '),
        tasks: trackTasks,
        status: trackTasks.every((task) => task.status === 'completed')
          ? 'completed'
          : 'in_progress',
      });
    }
    ...
    thoughts.forEach((thought) => {
      ...
      if (targetTask) {
        ...
        const checklist = [
          ...
          ...(thought.metadata?.external_refs ?? []),
        ];
        ...
        const syntheticTask: DevOpsTask = {
          ...
          checklist: thought.metadata?.external_refs,
