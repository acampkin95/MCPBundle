...
export class KeycloakAuthService {
  ...
  private async clientCredentialsFlow(): Promise<TokenSet> {
    ...
    // Make actual Keycloak token request
    ...
    if (!response.ok) {
      ...
      throw new Error(`Keycloak authentication failed: HTTP ${response.status} - ${errorText}`);
    }
  ...
  private async passwordFlow(): Promise<TokenSet> {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Keycloak password auth failed: HTTP ${response.status} - ${errorText}`);
    }
  ...
  public async refreshToken(): Promise<TokenSet> {
    ...
    try {
      ...
      // Make actual token refresh request
      const response = await fetch(tokenEndpoint, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
      ...
      const newTokenSet: TokenSet = {
        ...
        expiresAt: Date.now() + data.expires_in * 1000,
        tokenType: data.token_type,
        scope: data.scope || this.currentTokenSet.scope,
      };
  ...
  public async revoke(): Promise<void> {
    ...
    try {
      ...
      // Make actual token revocation request
      ...
      if (!response.ok) {
        logger.warn(`Token revocation failed: HTTP ${response.status}`);
      }
