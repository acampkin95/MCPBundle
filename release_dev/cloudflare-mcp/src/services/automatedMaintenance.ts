...
export class AutomatedMaintenanceService {
  ...
  private registerDefaultTasks(): void {
    ...
    this.tasks.set('log-rotation', {
      ...
      config: {
        logPaths: ['/var/log/nginx/*.log', '/var/log/postgresql/*.log', '/var/log/keycloak/*.log'],
        keepDays: 30,
      },
    });
    ...
    this.tasks.set('disk-cleanup', {
      ...
      config: {
        cleanPaths: ['/tmp', '/var/tmp', '/var/log/journal'],
        olderThanDays: 7,
      },
    });
  }
  ...
  private async runLogRotation(task: MaintenanceTask): Promise<string> {
    ...
    try {
      const result = await this.runner.run(`logrotate --force /etc/logrotate.d/server-mcp`, {
        requiresSudo: true,
        timeoutMs: 60000,
      });
      ...
      const commands = logPaths.map((path) => `find ${path} -type f -mtime +${keepDays} -delete`);

      const results = await Promise.all(
        commands.map((cmd) => this.runner.run(cmd, { requiresSudo: true, timeoutMs: 30000 }))
      );

      const totalDeleted = results.reduce((sum, r) => sum + (r.stdout.split('\n').length - 1), 0);
  ...
  private async runDiskCleanup(task: MaintenanceTask): Promise<string> {
    ...
    const results = await Promise.all(
      commands.map((cmd) => this.runner.run(cmd, { requiresSudo: true, timeoutMs: 60000 }))
    );
