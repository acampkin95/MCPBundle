...
export class PostgresManagerService {
  ...
  public constructor(
    ...
    this.pool = new Pool(
      connectionConfig ?? {
        ...
        user: process.env.POSTGRES_USER ?? 'mcp_admin',
        password: process.env.POSTGRES_PASSWORD,
        database: process.env.POSTGRES_DB ?? 'mcp_ecosystem',
  ...
  public async reindexDatabase(database: string): Promise<ReindexResult> {
    ...
    try {
      ...
      const result = await this.runner.run(
        `psql -d ${safeDatabase} -c "REINDEX DATABASE ${safeDatabase}"`,
        {
          requiresSudo: false,
  ...
  public async createBackup(database: string, destination: string): Promise<BackupResult> {
    ...
    try {
      ...
      await this.runner.run(`pg_dump -d ${safeDatabase} | gzip > ${safeBackupPath}`, {
        requiresSudo: false,
