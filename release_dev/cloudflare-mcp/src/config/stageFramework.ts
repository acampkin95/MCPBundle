...
const BUILTIN_STAGE_FRAMEWORK: StageFrameworkConfig = {
  ...
  transitions: [
    ...
    },
    {
      ...
      prompt:
        'Are the proposed options ready for decision making or do they need further analysis?',
    },
    {
      from: 'conclusion',
...
const parseExternalConfig = (path: string): StageFrameworkConfig | null => {
  if (!existsSync(path)) {
    logger.warn('Stage framework config path not found, falling back to built-in defaults', {
      path,
    });
    return null;
  }
